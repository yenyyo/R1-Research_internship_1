...
def general_info(tcr_df):
    # Assuming tcr_df is your DataFrame
    # Let's say you want to print the general information about it

    # Size of the DataFrame
    df_size = tcr_df.size

    # Number of columns
    num_columns = len(tcr_df.columns)
    # Maximum value in the column 'count'
    max_count = tcr_df['count'].max()
    # Minimum value in the column 'count'
    min_count = tcr_df['count'].min()
    # Number of unique occurrences in the DataFrame
    num_unique = tcr_df["cdr3"].nunique()
    # Number of rows
    num_rows = len(tcr_df)

    # Printing the general information
    print("General Information about tcr_df:")
    print("=================================")
    print(f"Size of the DataFrame: {df_size}, number of columns = {num_columns}, number of rows = {num_rows}")
    print(f"Maximum value in column 'count': {max_count}, Minimum value in column 'count': {min_count}")
    print(f"Number of unique occurrences: {num_unique}")
    ...
    for idx, sample_id in enumerate(list_files, start=1):
        ...
        try:
            ...
            for chunk in chunk_iter:
                matching_cdr3 = chunk[chunk["cdr3_amino_acid"].isin(tcr_df["cdr3"])].copy()
                if not matching_cdr3.empty:
